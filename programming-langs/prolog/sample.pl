...
tryUp(Maze, List, Row, Column, NextRow, NextColumn) :-
    NextRow is Row - 1,
    NextColumn is Column, NextRow > 0,
    maze(Maze, NextRow, NextColumn, open),
    not(inList([NextRow, NextColumn], List)).
...
tryDown(Maze, List, Row, Column, NextRow, NextColumn) :-
    NextRow is Row + 1, NextColumn is Column,
...
tryLeft(Maze, List, Row, Column, NextRow, NextColumn) :-
    NextRow is Row, NextColumn is Column - 1,
    NextColumn > 0,
    maze(Maze, NextRow, NextColumn, open),
    not(inList([NextRow, NextColumn], List)).
...
% Side Effects: None.
isGoal(Maze, Row, Column) :-
    mazeSize(Maze, Rows, Columns), Row is Rows, Column is Columns.
...
% Side Effects: stdout if true.
solve(Maze) :-
    move(Maze, [], MoveList, 1, 1),
    printList(MoveList),
    printMaze(Maze, MoveList).%
